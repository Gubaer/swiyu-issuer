...
- In `api/migrations/`, files use the convention `<DATE>_<SEQ>_<description>.sql` (e.g. `20260507_000003_issued_credentials.sql`). sqlx parses **only the leading numeric run before the first underscore** as the migration version — the `_000003` sequence number is part of the description, not the version. Two files with the same date prefix collide on `_sqlx_migrations.version` and fail at apply time with a `23505` duplicate-key error inside every `sqlx::test`. When adding a migration, pick a date prefix no existing migration uses. The sequence number is human-ordering only.
