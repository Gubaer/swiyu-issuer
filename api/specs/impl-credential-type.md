...
Newtype following the project bs58/prefix convention. Prefix: `ctype`. Wire form: `ctype_<bare-base58>`. Matches every other identifier in the codebase and decouples the URL from any externally meaningful string. This is the identifier shape the aspect spec now specifies (see [`aspect-credential-type.md`](aspect-credential-type.md) § *Properties → Identity*): a single globally unique bs58 value, not a `(tenant_id, slug)` composite and not the `vct`.
...
- `revocation_mode` is `TEXT` with values `revocable` / `suspendable` / `revocable_and_suspendable` / `none`, per enum-as-text convention. The value **drives runtime behaviour**: the credential-lifecycle handlers (`POST /issued-credentials/{id}/suspend`, `…/unsuspend`, `…/revoke`, per [`impl-credential-management.md`](impl-credential-management.md)) reject any call whose verb is not permitted by the credential type's mode — e.g. a `revoke` against a credential of a type marked `suspendable` returns HTTP 409. All four mode values are supported, per [`aspect-credential-type.md`](aspect-credential-type.md) § *Properties → Issuance behaviour*.
...
- **Disclosure-policy column.** `CredentialType` deliberately carries no field controlling which claims are selectively disclosable. The partition is fixed — registered envelope claims stay cleartext, every business claim is selectively disclosable — and is not configurable per type, per `vct` author, or per issuer; see [`aspect-credential-type.md`](aspect-credential-type.md) § *Selective disclosure*. The SD-JWT assembly that realises this lives in [`impl-credential-management.md`](impl-credential-management.md), not here.
...
- **Templating / standard-type propagation.** Bulk-publishing a "standard" credential type from a cantonal organisation into many tenants' rows is operational tooling, not a domain feature; out of scope per [`aspect-credential-type.md`](aspect-credential-type.md) § *Worked examples* (Example 2).
