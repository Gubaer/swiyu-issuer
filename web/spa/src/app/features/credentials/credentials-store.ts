...
// Tracks per-issuer paging state for the credentials table. The "intended"
// issuer id is the source-of-truth tag: every request reads it on dispatch, and
// the response is only committed if the tag still matches when it arrives. That
...
export class CredentialsStore {
  ...
  private readonly itemsSignal = signal<Credential[]>([]);
  ...
  loadFor(issuerId: string): void {
    ...
    this.service.list(issuerId).subscribe({
      ...
      error: () => {
        ...
        this.errorSignal.set('Could not load credentials.');
        this.loadingSignal.set(false);
      },
    });
  }
  ...
  loadMore(): void {
    ...
    this.service.list(issuerId, { cursor }).subscribe({
      ...
      error: () => {
        ...
        this.errorSignal.set('Could not load more credentials.');
        this.loadingSignal.set(false);
      },
    });
  }
  ...
  // Replace the matching row with the server's updated record (state, expired,
  // etc.). The lifecycle actions return the full record, so patching with the
  // whole object reflects every field the transition touched.
